...
scaler.fit(df_NoCode.astype('float64'))
...
def data_prepare(dataframe, name):
    ...
    df = scaler.transform(df.astype('float64'))
    X, y = split_sequences(df, n_steps)
    well_code = well_code[:len(y)]
    print('%s shape:' % name, X.shape, y.shape, well_code.shape)
    return X, y, well_code, df


X_1C, y_1C, well_code_1C, df_1C = data_prepare(df, 'NO 15/9-F-1 C')

X_11H, y_11H, well_code_11H, df_11H = data_prepare(df, 'NO 15/9-F-11 H')

X_12H, y_12H, well_code_12H, df_12H = data_prepare(df, 'NO 15/9-F-12 H')

X_14H, y_14H, well_code_14H, df_14H = data_prepare(df, 'NO 15/9-F-14 H')

X_15D, y_15D, well_code_15D, df_15D = data_prepare(df, 'NO 15/9-F-15 D')
...
model = lstm_model('lstm_All10_all', train=False)
...
def test_model(name):
    ...
    cust = np.mean(np.abs(y_pred - y.reshape(len(y), 1)) / (
                y.reshape(len(y), 1) + mean[-1] / std[-1]))
