...
def lstm_model(model_name, train=False):
    if train:
        # define model
        model = Sequential()
        model.add(GRU(50, activation='tanh', input_shape=(n_steps, n_features)))
        model.add(Dense(1))
...
X_12H_train, X_12H_test, y_12H_train, y_12H_test, mean_12H, std_12H = data_prepare(df, 'NO 15/9-F-12 H', 0.2)
X_14H_train, X_14H_test, y_14H_train, y_14H_test, mean_14H, std_14H = data_prepare(df, 'NO 15/9-F-14 H', 0.2)
X_15D_train, X_15D_test, y_15D_train, y_15D_test, mean_15D, std_15D = data_prepare(df, 'NO 15/9-F-15 D', 0.2)

X_train = concatenate((X_1C_train, X_11H_train, X_12H_train, X_14H_train, X_15D_train), axis=0)
y_train = concatenate((y_1C_train, y_11H_train, y_12H_train, y_14H_train, y_15D_train), axis=0)

X_test = concatenate((X_1C_test, X_11H_test, X_12H_test, X_14H_test, X_15D_test), axis=0)
y_test = concatenate((y_1C_test, y_11H_test, y_12H_test, y_14H_test, y_15D_test), axis=0)
...
for i in range(1, 11):
    ...
    model = lstm_model('lstm_EACH_EACH_GRU', train=True)
...
#
# fintune('NO 15/9-F-12 H', model_12H, 'lstm_EACH_EACH_12H', train=False, epoch=20)
#
# fintune('NO 15/9-F-14 H', model_14H, 'lstm_EACH_EACH_14H', train=False, epoch=20)
#
# fintune('NO 15/9-F-15 D', model_15D, 'lstm_EACH_EACH_15D', train=False, epoch=0)
