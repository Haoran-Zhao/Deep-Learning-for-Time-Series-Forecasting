...
dataset_235 = well_p_2.append([well_p_3, well_p_5], ignore_index=True)
dataset = dataset_235.values

dataset_4 = well_p_4.dropna(subset=['AVG_ANNULUS_PRESS']).values
X_test_4 = dataset_4[:, [8, 9, 10, 11, 13, 15, 16, 17]]
Y_test_4 = dataset_4[:, 12]
Y_test_4 = np.reshape(Y_test_4, (-1, 1))

X = dataset[:, [8, 9, 10, 11, 13, 15, 16, 17]]
Y = dataset[:, 12]
Y = np.reshape(Y, (-1, 1))
scaler = MinMaxScaler()
print(scaler.fit(X))
