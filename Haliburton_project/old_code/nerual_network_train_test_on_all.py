...
dataframe = dataframe.drop(columns=['AVG_CHOKE_UOM'])
...
df_4 = well_p_4.dropna(subset=['AVG_ANNULUS_PRESS'])
index_annulus_zeros = df_4.index[df_4['AVG_ANNULUS_PRESS'] == 0].tolist()
df_4 = df_4.drop(index_annulus_zeros)
df_all = well_p_2.append([well_p_3, df_4], ignore_index=True)
dataset = df_all.values
X = dataset[:, [8, 9, 10, 11, 13, 15, 14, 16, 17]]
Y = dataset[:, 12]
Y = np.reshape(Y, (-1, 1))
scaler = MinMaxScaler()
...
with open("model_train_test_on_all.json", "w") as json_file:
    json_file.write(model_json)
# serialize weights to HDF5
model.save_weights("model_train_test_on_all.h5")
