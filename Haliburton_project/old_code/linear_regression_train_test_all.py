...
dataset_235 = well_p_2.append([well_p_3, well_p_5, dataset_4], ignore_index=True)
dataset = dataset_235.values

X = dataset[:, [8, 9, 10, 11, 13, 14, 15, 16, 17]]
Y = dataset[:, 12]
Y = np.reshape(Y, (-1, 1))
...
dataset_1 = dataset_4.values
X_test_4 = dataset_1[:, [8, 9, 10, 11, 13, 14, 15, 16, 17]]
Y_test_4 = dataset_1[:, 12]
Y_test_4 = np.reshape(Y_test_4, (-1, 1))

scaler_1 = MinMaxScaler()
print(scaler_1.fit(X_test_4.astype(float)))
xscale_1 = scaler_1.transform(X_test_4)

X_test_1, _, Y_test_1, _ = train_test_split(xscale_1, Y_test_4, random_state=seed)

score_1 = model.evaluate(X_test_1, Y_test_1)
print(model.metrics_names)
print('mse_1: %.2f' % (score_1[0]))
print('r2_1 accuracy: %.2f%%' % (score_1[1]*100))
