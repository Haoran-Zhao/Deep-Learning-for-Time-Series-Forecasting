...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\State Shuffling the time series data
		\State Splitting first 80\% data as training set and rest 20\% data as test set
		\State Calculating mean and standard deviation of the training data
		\State Normalizing all the data based on mean and standard deviation
	\end{algorithmic}
\end{algorithm}
 ...
 \begin{adjustbox}{width=\columnwidth,center}
 		...
 		\begin{tabular}{lcccccccccccccc}
			...
			\multicolumn{1}{l}{Metrics}&\multicolumn{1}{l}{ $R^2$} &\multicolumn{1}{r}{ $RMSE$} &\multicolumn{1}{l}{ $R^2$} &\multicolumn{1}{r}{ $RMSE$}&\multicolumn{1}{l}{ $R^2$} &\multicolumn{1}{r}{ $RMSE$}&\multicolumn{1}{l}{ $R^2$} &\multicolumn{1}{r}{ $RMSE$}&\multicolumn{1}{l}{ $R^2$} &\multicolumn{1}{r}{ $RMSE$}&\multicolumn{1}{l}{ $R^2$} &\multicolumn{1}{r}{ $RMSE$}&\multicolumn{1}{l}{ $R^2$} &\multicolumn{1}{r}{ $RMSE$}\tabularnewline
 			\hline
 			11H & 0.96 & 0.19 & ---&---  & ---&--- &0.95 & 0.18 & ---&--- & 0.96&0.14 & 0.94& 0.06 \tabularnewline
 			1C & ---&--- & 0.93 & 0.26& ---&--- & 0.88 & 0.15& ---&--- &0.92 &0.11&0.78&0.06\tabularnewline
 			15D &--- &--- & ---&---  & 0.82 & 0.41 & ---&---  & ---&--- &0.65&0.06&-0.23&0.04\tabularnewline
 			12H &  ---& ---& ---&---  & ---&---  & ---&---  & 0.99& 0.09&---&---&0.99 &0.09\tabularnewline
 			14H & ---&--- & ---&---  & ---&---  & ---&---  & 0.99&0.07&---&---&0.99&0.08\tabularnewline
 			Overal &\cellcolor{green!40} 0.96 & 0.19 & 0.93 & 0.26 & 0.82 & 0.41 & \cellcolor{green!40}0.96 & 0.17 &\cellcolor{green!40}0.99 &0.08 &\cellcolor{green!40}0.98&0.11 &0.99&0.08\tabularnewline
 			 			\hline
 	\end{tabular}
	\end{adjustbox}
 \end{table}
...
%%\hl{an introduction about LSTM model (I already included some previous explanation you had sent me), why we are motivated to use this model, what are the hyper parameters or  tuning parameters, how did we create test and training sets, what is the method of validation , the results should come in something like tables below}

%%%\par \hl{d:number of days used for creating time series data, p1 : tuning parameter that we have for LSTM like number of layers and..., p2: same as p1}
...
For the LSTM studies, we investigated the same 7 combinations of datasets as SVR. To tune the model, we trained the model over different number of time -steps , which is 3, 5, 10, 20, different number of neurons and epochs. The results of hyper-parameter tuning are shown in Table \ref{tb:svr_hyper}. The model evaluation results of each well and the overall results of oil production prediction using LSTM are shown in Table \ref{tb:rs_oil_lstm}. To minimize the randomness, the results in Table \ref{tb:rs_oil_lstm} is the average out of 10 trials. From the first 5 cases in Table \ref{tb:rs_oil_lstm}, we can see that the training results is based on how well the dataset is. As mentioned in the previous section, the best combination of dataset is that of {11H, 1C, 15D} and{12H, 14H}. We can conclude that when the oil production rates of wells are in a similar range, better prediction results can be achieved on each well and overall results. In the last case, we can conclude that, although the training results are good for some wells, the range and magnitude of well production rates significantly affects results. 
 ...
 \begin{adjustbox}{width=\columnwidth,center}
 		...
 		\begin{tabular}{lcccccccccccccc}
			...
			\multicolumn{1}{l}{Metrics}&\multicolumn{1}{l}{ $R^2$} &\multicolumn{1}{r}{ $RMSE$} &\multicolumn{1}{l}{ $R^2$} &\multicolumn{1}{r}{ $RMSE$}&\multicolumn{1}{l}{ $R^2$} &\multicolumn{1}{r}{ $RMSE$}&\multicolumn{1}{l}{ $R^2$} &\multicolumn{1}{r}{ $RMSE$}&\multicolumn{1}{l}{ $R^2$} &\multicolumn{1}{r}{ $RMSE$}&\multicolumn{1}{l}{ $R^2$} &\multicolumn{1}{r}{ $RMSE$}&\multicolumn{1}{l}{ $R^2$} &\multicolumn{1}{r}{ $RMSE$}\tabularnewline
 			\hline
 			11H &0.96 &0.04 &--- &--- &--- &---  &0.97 & 0.01&--- &---  &0.96 &0.02&0.91 &0.08 \tabularnewline
 			1C &--- &--- &0.91&0.07 &--- &--- &0.92&0.01 &--- &--- &0.92&0.01&0.71 &0.74\tabularnewline 
 			15D &--- &--- &--- &--- &0.84&0.13 &--- &--- &--- &--- &0.77&0.003 &0.87& 0.09\tabularnewline
 			12H & --- &--- &--- &--- &--- &--- &--- &--- &0.99 &0.007&--- &---&0.11 &0.90\tabularnewline
 			14H &--- &--- &--- &--- &--- &--- &--- &--- &0.99&0.005  &--- &--- &0.83&0.13 \tabularnewline
 			Overal &\cellcolor{green!40}0.96&0.04 &0.91 &0.07 &0.84 &0.13&\cellcolor{green!40} 0.98& 0.01 &\cellcolor{green!40}0.99 &0.006 &\cellcolor{green!40}0.98 &0.01&0.87&0.11 \tabularnewline
 			 			\hline
 	\end{tabular}
	\end{adjustbox}
 \end{table}
