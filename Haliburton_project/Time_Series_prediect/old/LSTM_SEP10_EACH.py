...
def test_model(name, mean, std):
    ...
    print('%s : mse = ' % name, mse0, 'r2 = ', r20, 'cust = ', cust0)
    r2_list.append(r2)
    mse_list.append(mse)
...
def data_prepare(dataframe, name, ratio):
    ...
    X, y = shuffle(X, y)
    X_train0, X_test0, y_train0, y_test0 = X[int(ratio * len(df0)):, :], X[:int(0.2 * len(df0)), :], y[int(0.2 * len(df0)):], y[:int(0.2 * len(df0))]
...
model = lstm_model('lstm_SEP10_EACH', train=False)
